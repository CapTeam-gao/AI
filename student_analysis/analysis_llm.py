...
- 구현 범위, 기능 간 연결, 주도성, 구조화, 문제 해결 경험을 종합해서 평가하라.
- 반드시 완성된 서비스나 프로젝트 단위 경험만 높게 평가하지 마라. 프로젝트 전체가 아니어도 직접 구현한 기능, 구조, 환경, 알고리즘, 디자인 시스템, 데이터 흐름이 충분히 복잡하고 수준 높으면 높은 구현 경험으로 평가할 수 있다.
- 완성된 프로젝트명이나 배포 경험이 부족해도 상태 관리, 인증, DB 모델링, 마이그레이션, 네트워크 정책, 세이브 구조, UI 컴포넌트 시스템, 레이아웃 문제 해결처럼 독립적인 구현 난도가 높고 해결 과정이 구체적이면 해당 깊이를 인정하라.
- 보안, 인프라, 네트워크, Kubernetes 경험은 웹/앱 기능 구현과 다른 형태의 고급 구현 경험으로 평가하라.
- VLAN, 방화벽, NAT, HA, Docker Compose, Kubernetes CNI, 네트워크 정책, 취약점 검증처럼 환경을 직접 구성하고 동작을 검증한 경험은 단순 도구 나열이 아니라 실전 구현 근거로 본다.
- 다만 강의나 실습이라는 표현이 있어도 직접 구성, 정책 적용, 트래픽 관찰, 취약점 재현/검증, 문제 해결 과정이 적혀 있으면 낮게 단정하지 마라.
- 경험에 직접 적힌 내용만 근거로 사용하라. 가능성, 추정, 일반적인 학습 경로는 근거가 아니다.
- 이 평가는 현업 개발자 채용 평가가 아니라 학생 캡스톤 팀 매칭을 위한 상대 평가다. 팀에서 핵심 구현 파트를 맡길 수 있는 학생이면 상위 등급으로 볼 수 있다.
- 성능 개선, 운영, 장애 대응, 수치 기반 최적화는 있으면 강한 상위 근거로 사용하되, 없다는 이유만으로 상 등급에서 제외하지 마라.
- 9~10점은 난도 높은 기술/구조/환경을 여러 개 직접 구성했거나, 아키텍처 설계, 성능 개선, 운영/장애 대응, 복잡한 문제 해결 근거가 명확할 때 줄 수 있다.
- 7~8점은 여러 기능이 결합된 프로젝트나 난도 높은 핵심 기능/환경/파이프라인을 스스로 구축한 근거가 있을 때 줄 수 있다.
- 단순 구현, 간단한 서버, 간단한 agent, 클론코딩, 내장 데이터만 사용하는 따라하기식 실습은 6점 이하로 제한한다.

skill_level 판정:
- 상: 학생 기준에서 팀의 핵심 구현 파트를 맡길 수 있을 정도로 난도 높은 기술/구조/환경을 직접 여러 개 구성한 경우. 또는 프로젝트 전체가 아니더라도 RAG/Agent/LLM 서빙/파인튜닝, 인증/DB/API 연동, 컨테이너/네트워크/보안 정책, Kubernetes, 취약점 분석처럼 복잡한 핵심 기능이나 환경을 직접 연결하고 검증한 근거가 있는 경우. 성능 개선, 운영, 장애 대응, 수치 기반 최적화는 강한 상 근거지만 필수 조건은 아니다.
- 중상: 여러 기능을 연결한 실제 프로젝트를 구현했거나, 하나의 영역에서 구조화된 구현과 의미 있는 문제 해결 경험이 있는 경우. 다만 고급 기술/환경 구성이 단일 실습이나 부분 구현에 가깝고 팀의 핵심 구현 파트를 주도할 근거가 제한적이면 중상으로 본다.
- 중: 구체적인 프로젝트 구현 경험은 있으나 구조화, 복합 기능 연결, 고급 기술 적용 근거가 부족한 경우.
...
class StudentAnalysis(BaseModel):
    ...
    8점: 여러 기능이나 도구를 연결해 구조화된 프로젝트, 파이프라인, 서비스 환경을 직접 구성한 경험이 뚜렷함
    9점: 난도 높은 기술/구조/환경을 여러 개 직접 구성했거나 아키텍처 설계, 성능 개선, 운영/장애 대응, 테스트 자동화 등 고급 경험이 명확함
    ...
    - 고급 기술 이름이 있어도 간단한 따라하기식 실습이면 낮은 점수를 준다.
    - 완성된 프로젝트 단위가 아니어도 직접 구현한 기능, 구조, 환경, 알고리즘, 디자인 시스템, 데이터 흐름이 복잡하고 구체적이면 높은 점수를 줄 수 있다.
    - 프로젝트명이나 배포 경험보다 실제로 무엇을 설계하고 구현했는지, 어떤 문제를 해결했는지, 그 구현이 얼마나 독립적이고 재사용 가능한지를 우선 평가한다.
    - 보안, 인프라, 네트워크, Kubernetes 경험은 직접 구성한 범위와 검증한 결과를 기준으로 평가한다.
    - VLAN, 방화벽, NAT, HA, Docker Compose, Kubernetes CNI, 네트워크 정책, 취약점 검증처럼 실제 환경 구성과 동작 확인이 적혀 있으면 실전 구현 근거로 인정한다.
    ...
    상:
    - 학생 기준에서 팀의 핵심 구현 파트를 맡길 수 있을 정도로 난도 높은 기술/구조/환경을 직접 여러 개 구성한 경험이 있는 경우
    - 프로젝트 전체가 아니어도 핵심 기능/구조/환경을 직접 설계하고 구현하거나 여러 도구를 연결해 동작까지 검증한 근거가 있는 경우
    - 성능 개선, 운영, 장애 대응, 수치 기반 최적화는 필수 조건이 아니라 강한 가산 근거로 본다
    중상:
    - 여러 기능을 연결한 프로젝트와 구조화 또는 의미 있는 문제 해결 경험이 있음
    - 고급 기술을 사용했지만 단일 실습, 부분 구현, 따라하기식 구성에 가깝거나 팀의 핵심 파트를 주도할 정도의 근거는 부족한 경우
    중: 
    - 구체적인 프로젝트 구현 경험은 있으나 구조화, 복합 기능 연결, 고급 기술 적용 근거가 부족함
    중하:
    - 단일 기능이나 작은 범위의 구현 경험으로 범위, 깊이, 주도성 근거가 제한됨
    하: 
    - 기초 사용이나 실습 수준이거나 구체적인 구현 근거가 부족함
    - 완성된 프로젝트가 아니어도 직접 구현한 기능/구조/환경/디자인 시스템/데이터 흐름의 난도와 문제 해결 근거가 높으면 상위 등급으로 평가할 수 있음
    - 보안/인프라/네트워크/Kubernetes 분야는 직접 구성한 환경, 적용한 정책, 검증한 통신 흐름, 발견하거나 재현한 취약점을 구현 근거로 평가함
    - 이 평가는 현업 개발자 기준이 아니라 학생 캡스톤 팀 매칭 기준임
