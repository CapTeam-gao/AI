...
skill_level 판정:
- 상: 복잡한 프로젝트에서 설계, 주도적 구현, 복잡한 문제 해결과 최적화 또는 운영 경험이 명확한 경우. 보안/인프라 분야에서는 네트워크 분리, 방화벽/HA, 컨테이너 오케스트레이션, 정책 검증, 취약점 분석을 복합적으로 직접 구성하고 검증한 근거가 이에 해당할 수 있다.
- 중상: 여러 기능을 연결한 실제 프로젝트를 구현했고 구조화 또는 의미 있는 문제 해결 경험이 있는 경우. 보안/인프라 분야에서는 Docker Compose, 서버 구성, VLAN/DMZ, Kubernetes 네트워크 정책, 웹 취약점 검증 등 여러 영역을 직접 구성하고 동작을 확인한 근거가 이에 해당할 수 있다.
